...
from vues.vue import MenuRapportJoueur, MenuRapportTournoi
from vues.vue import MenuTournoi, MenuJoueur, MenuTour
...
class TournoiManager:
    ...
    def ajout_joueurs(self):
        ...
        Returns:
            liste:
            une liste des joueurs avec pour chacun
            une liste de leurs informations
        ...
        try:
            ...
            self.tournoi.update_joueurs_tournoi_database(
                tournoi=tournoi_choisi, joueurs=dict_joueurs
            )
    ...
    def tri_joueurs_classement_mondial(self):
        ...
        joueurs_triés = sorted(
            joueurs_unserialized(), reverse=True, key=lambda joueur: joueur[4]
        )
        return joueurs_triés

    def tri_joueurs_points_tournoi(self):
        """
        Cette méthode trie les joueurs en fonction
        des points qu'ils ont gagnés durant le tournoi

        Returns:
            tableau:
            les joueurs sont triés du nombre de points
            le plus faible au plus élevé
        """
        self.tournoi.joueurs = tournois_database.get(where("nom") == self.tournoi.nom)[
            "joueurs"
        ]
    ...
    def commencer_premier_tour(self):
        ...
        on enregistre le temps et la date de début/fin du tour
        ...
        premier_tour = Tour(
            ...
            liste_matchs=resultat_premiers_matchs,
            numero_round=1,
        )
    ...
    def commencer_tour_suivant(self, nb_round):
        ...
        tour_suivant = Tour(
            date_heure_debut=debut_temps,
            date_heure_fin=fin_temps,
            liste_matchs=resultat_tour_suivant,
            numero_round=nb_round,
        )
    ...
    def modifier_tournoi(self):
        ...
        menu_tournoi = MenuTournoi()
        choix_valide = False
        nom_tournoi = input("Quel tournoi voulez-vous modifier ?(nom)\t")
        try:
            ...
            while choix_valide is False:
                ...
                # modification du lieu
                    ...
                    tournois_database.update(
                        {"lieu": lieu}, where("nom") == nom_tournoi
                    )
                    print()
                    print("**** modification réussie ****")
                    choix_valide = True
                ...
                # modification de la date
                    ...
                    tournois_database.update(
                        {"date": date_tournoi}, where("nom") == nom_tournoi
                    )
                    print()
                    print("**** modification réussie ****")
                    choix_valide = True
                ...
                # modification du temps de jeu
                    ...
                    tournois_database.update(
                        {"temps": temps}, where("nom") == nom_tournoi
                    )
                    print()
                    print("**** modification réussie ****")
                    choix_valide = True
                ...
                # modification de la description
                    ...
                    tournois_database.update(
                        {"description": description}, where("nom") == nom_tournoi
                    )
                    print()
                    print("**** modification réussie ****")
                    choix_valide = True
...
class MatchManager(TournoiManager):
    ...
    def creer_premiers_matchs(self, joueurs_triés):
        ...
        for un_match in range(nb_matchs):
            un_match = Match(
                ...
                joueur2=joueurs_triés[indice_joueur_milieu],
                numero=match_numero,
            )
            match_numero += 1
    ...
    def resultat_match(self, matchs):
        ...
        for match in matchs:
            ...
            joueur1 = match_serialized["joueur1"]
            joueur2 = match_serialized["joueur2"]
            ...
            for joueur in joueurs:
                nom_joueur = joueur["prenom"] + " " + joueur["nom"]
                if nom_joueur == match_serialized["joueur1"]:
                    joueur["score"] += match_serialized["score_joueur1"]
                    joueurs_modifies.append(joueur)
                elif nom_joueur == match_serialized["joueur2"]:
            ...
            tournois_database.update(
                {"joueurs": joueurs_modifies}, where("nom") == self.tournoi.nom
            )
...
class RapportJoueurManager:
    ...
    def lancer_rapport(self):
        ...
        if choix == "1":
            ...
            try:
                joueur = Joueur(
                    **joueurs_database.get(
                        where("nom") == nom and where("prenom") == prenom
                    )
                )
            except TypeError:
...
class RapportTournoiManager:
    def lancer_rapport(self):
        ...
        if choix == "1":
            ...
            try:
                tournoi_unserialized = Tournoi(
                    **tournois_database.get(where("nom") == nom_tournoi)
                )
            except TypeError:
                return print("\nLe tournoi ne se trouve pas dans la base de données\n")
            ...
            if choix_valide is False:
                ...
                if choix_tri == "1":
                    choix_valide = True
                    try:
                        joueurs_tri_alphabetique = sorted(
                            joueurs, key=lambda joueur: joueur["nom"]
                        )
                ...
                # on trie les joueurs selon leurs classements mondial
                    ...
                    choix_valide = True
                    try:
                        joueurs_tri_score = sorted(
                            joueurs, key=lambda joueur: joueur["classement"]
                        )
                        tournoi_unserialized.joueurs = []
                        for joueur in joueurs_tri_score:
                            joueur_serialized = Joueur(**joueur)
...
class ModifierJoueur(JoueurManager):
    ...
    def modifier_prenom(self):
        ...
        tournois_database.update(
            {"joueurs": self.joueurs_tournoi}, where("nom") == self.tournoi["nom"]
        )
